...
uniform float _filter_4;//div for _check_distance_limit for thin lines
...
void main()
{
	...
	if (_my_material < 100.)	{discard;};//ignored
	...
	if (_my_dist_to_cam > _check_distance_limit * 0.25)	{discard;};//too far
			...
			for(float _y = -_check_range; _y < _check_range + 1.; _y += 1.)
				{
					...
					float _continue = 0.;
					if (_check_material < 100.)	{_continue = 1.;}
					else{	if (_check_dist < _check_distance_limit)		{_continue = 1.;};	};
