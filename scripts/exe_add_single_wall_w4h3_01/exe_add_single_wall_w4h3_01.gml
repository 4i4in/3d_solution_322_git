function exe_add_single_wall_w4h3_01(_initial_pm)
{
	//3d obj start
	var _temp_o = {};
	...
	_temp_o.character_sheet = {		name: "wall_w4h3_01", hash: exe_return_hash(	string_join("|",current_time,date_current_datetime(),string(string_length(map3d)))	),
									file_name : "openwall_swapable_01.txt", file_directory : "terrain/",};
	_temp_o.control_method = {};
	//_temp_o.camera =[];
	
	struct_set(_temp_o.character_sheet, "render_pass_sorting",		"static_major"	);
	struct_set(_temp_o.character_sheet, "items_swaps",		"wall_w4h3_items_swaps_01"	);
	
	_temp_o.part_naming =						[	"main_part",										"decorations",											];
	...
	_temp_o.part_vbuff_load_reference =			[	["vbuff_wall_01_w4h3_show","1.","N"],				["vbuff_empty","1.","N"],								];
	_temp_o.part_texture_shader_setings =		[	[7.,1.,0.],											[101.,5.,0.],			];	//material true/false
	
	_temp_o.part_vbuff_colider_load_reference = [	["vbuff_wall_01_w4h3_colider","1.","N"],			["vbuff_empty","1.","N"],								];
	
	_temp_o.part_texture_helper_load_reference =[	["tex_helpers","tex_normal_z"],						["tex_helpers","tex_normal_z"],							];
	_temp_o.part_vbuff_helper_load_reference =	[	["vbuff_dir_arrow","5.","N"],						["vbuff_dir_arrow","1.","N"],							];
