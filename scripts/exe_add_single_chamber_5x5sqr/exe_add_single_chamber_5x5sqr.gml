function exe_add_single_chamber_5x5sqr(_initial_pm)
{
	//3d obj start
	var _temp_o = {};
	...
	//conection point 4 is defualt
	if array_length(_initial_pm) == 16	
		{	
			var _delta = [0,0,0];
			_delta = exe_move_fb(_delta,_posMat,200.);
			//_delta = exe_move_updown(_delta,_posMat,0.);
			//_delta = exe_move_sides(_delta,_posMat,0.);
			_posMat = exe_delta_add_Mat(_delta,_posMat);
		};

	_temp_o.character_sheet = {		name: "chamber_5x5sqr_01", hash: exe_return_hash(	string_join("|",current_time,date_current_datetime(),string(string_length(map3d)))	),
									file_name : "chamber_swapable_01.txt", file_directory : "terrain/",};
	_temp_o.control_method = {};
	//_temp_o.camera =[];
	
	struct_set(_temp_o.character_sheet, "render_pass_sorting",		"static_major"	);
	struct_set(_temp_o.character_sheet, "items_swaps",		"chamber_items_swaps_5x5sqr_01"	);
	...
	_temp_o.part_vbuff_load_reference =			[	["vbuff_chamber_3x3h1_show","1.","N"],				["vbuff_empty","1.","N"],							["vbuff_empty","1.","N"],								["vbuff_empty","1.","N"],							["vbuff_empty","1.","N"],								["vbuff_empty","1.","N"],								];
	_temp_o.part_texture_shader_setings =		[	[7.,1.,0.],											[101.,5.,0.],										[7.,1.,0.],												[7.,1.,0.],											[7.,1.,0.],												[7.,1.,0.],												];	//material true/false
	
	_temp_o.part_vbuff_colider_load_reference = [	["vbuff_chamber_3x3h1_colider","1.","N"],			["vbuff_empty","1.","N"],							["vbuff_empty","1.","N"],								["vbuff_empty","1.","N"],							["vbuff_empty","1.","N"],								["vbuff_empty","1.","N"],								];
